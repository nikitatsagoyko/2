...
(function staticManagerDefinition() {
  ...
  var stManager = {
    ...
    _getLoadingState: function _getLoadingState(fileName) {
      ...
      // Â stVersions ìîæåò âîîáùå íå áûòü èíôîðìàöèè î íåêîòîðûõ ëåãàñè ôàéëàõ
...
window.Widgets = {
  ...
  popupBoxOpen: function(url, params, name, opts) {
    ...
    if (activePopup) {
      var closeTriggersWidgetsEnabled = partConfigEnabled('close_triggers_widgets');
      var closeType;
      ...
      if (closeTriggersWidgetsEnabled) {
          window.addEventListener('message', triggerCloseHandler);
      }

      activePopup.focus();
      opts.onClose && (function check() {
        !activePopup || activePopup.closed ? closeTriggersWidgetsEnabled ? onCloseInternal() : opts.onClose() : setTimeout(check, 1000);
      })();
    }
  },
...
function extractUrls(text, inactive) {
  ...
  while (text) {
    ...
    if (!matchesLiteUrls || !matchesLiteUrls.length || matchesLiteUrls[0][0] === '@') {
      // Ïðîâåðêà íà ñîâïàäåíèå ïåðâîãî ñèìâîëà äîáàâëåíà äëÿ èñêëþ÷åíèÿ ïî÷ò ñ íåñêîëüêî óðîâíåâûìè äîìåíàìè
      continue;
    }
